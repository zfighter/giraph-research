...
public class IdValueEdgesTextOutputFormat<I extends WritableComparable,
  ...
  /**
   * Vertex writer used with {@link IdValueEdgesTextOutputFormat}.
   */
  protected class IdWithValueVertexWriter extends TextVertexWriterToEachLine {
    ...
    protected Text convertVertexToLine(Vertex<I, V, E, ?> vertex)
      ...
      if (reverseOutput) {
        ...
        third = vertex.getNumEdges();
      } else {
        first = vertex.getId().toString();
        second = vertex.getValue().toString();
        third = vertex.getNumEdges();
      }
      Text line = new Text(first + ":" + third + delimiter + second);
      return line;
    }
